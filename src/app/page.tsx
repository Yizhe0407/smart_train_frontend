  ...
  return (
    <div className="container max-w-md mx-auto p-4">
      <Card className="mb-6">
        ...
        <CardContent>
          <div className="space-y-4">
            <div>
              <label htmlFor="county-select" className="block text-sm font-medium mb-1">縣市</label>
              <Select
                value={county}
                onValueChange={(value) => {
                  setCounty(value)
            ...
            <div>
              <label htmlFor="station-select" className="block text-sm font-medium mb-1">車站</label>
              <Select
                value={station}
                onValueChange={setStation}
                disabled={!county || currentStations.length === 0}
              >
                ...
                <SelectContent>
                  {currentStations.map((s) => (
                    <SelectItem key={s.name} value={s.name}>
                      {s.name} {s.isSmall && <span className="text-xs text-muted-foreground">(招呼站)</span>}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
                    ...
                    <PopoverContent className="w-auto p-0">
                      <Calendar
                        ...
                        disabled={(day) => {
                          ...
                          return day < today || day > thirtyDaysLater
                        }}
                        initialFocus
                      />
                    </PopoverContent>
            ...
            <Button className="w-full mt-4" onClick={handleSearch} disabled={!county || !station || isLoading}>
              {isLoading ? (
                <>
      ...
      {searchResults !== null && (
            ...
            {searchResults.length === 0 ? (
              ...
              <div className="space-y-3">
                {searchResults.map((train) => (
                  <Card key={train.trainNumber} className="shadow-sm hover:shadow-md transition-shadow">
                    <CardContent className="p-4">
                      <div className="flex justify-between items-center">
                        <div>
                          <div className="font-medium text-lg">{train.trainNumber} 次列車</div>
                          <div className="text-sm text-muted-foreground">預計抵達: {train.arrivalTime}</div>
                          <div className="text-sm text-muted-foreground">開往: {train.destination}</div>
                        </div>
                        <Button variant="outline" size="sm" onClick={() => handleBookTrain(train)} disabled={isLoading}>
                          預約此班次
                        </Button>
                      </div>
                    </CardContent>
       ...
       {/* 初始提示或無結果時的提示，如果 searchResults 為 null 且不在 loading */}
      {searchResults === null && !isLoading && (
        <div className="text-center text-muted-foreground py-12">
          <p>請選擇縣市、車站並點擊查詢按鈕以搜尋可預約班次。</p>
        </div>
      )}
        ...
        <DialogContent>
          ...
          {confirmDialog.train && (
            <div className="py-4 space-y-1">
              ...
              {!bookNow && date && (
                 <p>
                    <strong>預約日期:</strong> {format(date, "yyyy-MM-dd", { locale: zhTW })}
                 </p>
              )}
               {!bookNow && (
                 <p>
                    <strong>預約時間(約):</strong> {time}
                 </p>
              )}
            </div>
          )}
